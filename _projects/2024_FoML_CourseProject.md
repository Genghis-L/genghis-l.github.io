...
<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid
      ...
      title="Figure 1"
      caption="Increasing Complexity in Adversarial Training"
      zoomable=true
    %}
  </div>
</div>
...
<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid
      ...
      class="img-fluid rounded z-depth-1"
      title="Figure 2"
      caption="Trade-off Between $\mathcal{R}_{\text {nat }}(f)$ and $\mathcal{R}_{\text {bdy }}(f)$"
      zoomable=true
    %}
  </div>
</div>
...
\textbf{Theorem 3.2 [\cite{zhang_theoretically_2019}]: }
...
\textbf{TRADES Algorithm [\cite{zhang_theoretically_2019}]: }
...
Heuristically, [\cite{zhang_theoretically_2019}] use two heuristics to achieve more general defenses:
...
For multi-class problems, a surrogate loss is calibrated if minimizers of the surrogate risk are also minimizers of the $0-1$ risk [\cite{pires_multiclass_2016}]. Examples of multi-class calibrated loss include cross-entropy loss. Algorithmically, [\cite{zhang_theoretically_2019}] extend the problem to the case of multi-class classifications by replacing $\phi$ with a multi-class calibrated loss $\mathcal{L}(\cdot, \cdot)$ :
